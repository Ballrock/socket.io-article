...
Un fonctionnement simple mais pas simpliste qui permet de réaliser beaucoup de chose. 
Il est à noter qu'il existe dans le framework un système de "room" qui permet de réunir plusieurs sockets ensemble. Ce qui permet entre autre pour envoyer directement le message à tous les sockets de cette "room".
...
De là nous allons commencer à pouvoir définir les comportements lors de la réception de message spécifiques. Après un peu de développement nous obtenons quelque chose qui ressemble à ça :
...
Je vais survoler cette partie car au final, on retrouve exactement le même principe de réception et d'émit que coté serveur. La seul différence est que cette fois-ci on communique uniquement avec un acteur, le serveur.

``` bash
npm install --save socket.io-client
```

Une fois installé et inclus nos js/ts/html ce petit framework on initialise la connection :
...
Une fois cela réalisé on peut definir comme d'habitude nos chers callback :
...
Dernier point, il est très important de faire attention à la quantité d'information envoyé au client, notamment pour des questions de performances coté client. En effet, et c'est encore plus vrai pour un web-app, un message envoyant par exemple l'historique des coups joués, peut vite devenir énorme si beaucoup de monde est connecté et actif. Mais cette dernière remarque tient plus de la logique que du framework, j'en convient :)
